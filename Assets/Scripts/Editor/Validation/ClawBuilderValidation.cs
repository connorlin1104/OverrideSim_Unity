...
public static class ClawBuilderValidation
{
    ...
    private static void RunInteractive()
    {
        ...
        ValidationUtil.RunInteractive("Validate Build Claw", Run);
    }

    public static void RunBatchValidate() => ValidationUtil.RunBatch("Validate Build Claw", Run);
    ...
    private static string StructureAndTeardown()
    {
        ...
        foreach ((string id, string what) in new[] { (flipId, "flip"), (clampId, "clamp") })
        {
            ...
            ValidationUtil.Assert(mech != null, $"the {what} should be registered as a mechanism (id '{id}')");
            ValidationUtil.Assert(mech.type == RobotMechanisms.TypePneumatic,
                $"the {what} must register as a PNEUMATIC — that's what gives the player the " +
                "1-button/2-button choice for free");
            ValidationUtil.Assert(mech.pneumatic != null, $"the {what} needs a PneumaticActuator");
        }
        ...
        ValidationUtil.Near(flipAct.travelSeconds, FlipSeconds, 0.001f,
            "the flip should travel over the time the form asked for");
        ValidationUtil.Assert(clampAct.travelSeconds <= 0f,
        ...
        ValidationUtil.Assert(flipBody != null && flipBody.jointType == ArticulationJointType.RevoluteJoint,
            "the flip must be a revolute joint");
        ValidationUtil.Assert(flipBody.twistLock == ArticulationDofLock.LimitedMotion,
            "the flip must be limited-motion — a free-spinning flip would never stop at 180");
        ValidationUtil.Near(Mathf.Abs(flipBody.xDrive.upperLimit - flipBody.xDrive.lowerLimit), FlipAngle, 0.5f,
        ...
        ValidationUtil.Assert(openGrab != null, "a claw with grab enabled should have produced a ClawGrab");
        ValidationUtil.Assert(!openGrab.grabWhenRetracted,
        ...
        ValidationUtil.Assert(shutGrab != null && shutGrab.grabWhenRetracted,
        ...
        ValidationUtil.Assert(followerBody != null, "the second clamp half should have become a joint");
        ValidationUtil.Assert(f.jawB.GetComponent<PneumaticActuator>() == null,
            "the second clamp half is coupled, so it must NOT also carry an actuator");
        ValidationUtil.Assert(f.registry.Find(followerId) == null,
        ...
        ValidationUtil.Assert(coupler != null, "the second clamp half should be coupled to the first");
        ValidationUtil.Assert(coupler.mode == JointCoupler.CoupleMode.Position,
            "clamp halves track each other's ANGLE (a linkage), not speed");
        ValidationUtil.Assert(coupler.driver == f.jawA.GetComponent<ArticulationBody>(),
            "the second half should follow the driven half");
        ValidationUtil.Assert(coupler.follower == followerBody, "the coupler should drive its own body");
        ValidationUtil.Assert(coupler.ratio < 0f,
        ...
        ValidationUtil.Assert(f.jawA.transform.IsChildOf(f.flip.transform) && f.jawB.transform.IsChildOf(f.flip.transform),
            "both clamp halves must sit under the flip link, or they wouldn't flip with the claw");
        ValidationUtil.Assert(f.clampRod.transform.IsChildOf(f.flip.transform),
            "the clamp cylinder rides the claw, so it belongs under the flip link");
        ValidationUtil.Assert(!f.flipRod.transform.IsChildOf(f.flip.transform),
        ...
        ValidationUtil.Assert(rod != null && body != null, "both halves of the clamp cylinder need a slide follower");
        ValidationUtil.Assert(rod.slideUnits > 0f && body.slideUnits < 0f,
            $"the rod must extend and the body recoil the other way (got rod {rod.slideUnits}, " +
            $"body {body.slideUnits})");
        ValidationUtil.Near(rod.slideUnits - body.slideUnits, ClampStrokeMm / PneumaticSetup.MmPerUnit, 1e-3f,
            "however the recoil is split, the rod and body must stay one full stroke apart");
        ValidationUtil.Near(rod.slideUnits, -body.slideUnits, 1e-3f,
            "at recoil 0.5 the two halves should travel equal and opposite (the balanced case)");
        ValidationUtil.Assert(rod.progressBody == f.jawA.GetComponent<ArticulationBody>(),
            "the clamp cylinder should read its progress off the driven clamp joint");
        ValidationUtil.Assert(f.clampRod.GetComponent<ArticulationBody>() == null,
        ...
        foreach (Collider c in f.clampRod.GetComponentsInChildren<Collider>(true))
            ValidationUtil.Assert(!c.enabled, "a cosmetic cylinder's colliders must be off — a teleporting collider " +
                               "would fling game pieces");

        // --- Grab ----------------------------------------------------------------------------------
        ClawGrab grab = f.registry.GetComponentInChildren<ClawGrab>(true);
        ValidationUtil.Assert(grab != null, "the grab should have been wired");
        ValidationUtil.Assert(grab.clampPneumatic == f.registry.Find(clampId).pneumatic,
            "the grab must ride the CLAMP's piston, so closing the claw is grabbing");
        ValidationUtil.Assert(grab.holdPoint != null && grab.holdPoint.IsChildOf(f.flip.transform),
        ...
        ValidationUtil.Assert(mouth != null && mouth.isTrigger, "the claw mouth must be a trigger to catch pieces");
        ValidationUtil.Assert(grab.passThroughWhileHeld,
        ...
        ValidationUtil.Assert(grab.snapSpeed > 0f,
        ...
        ValidationUtil.Assert(grab.releaseGrace > 0f,
        ...
        ValidationUtil.Assert(!f.registry.GetComponentInChildren<ClawGrab>(true).passThroughWhileHeld,
        ...
        ValidationUtil.Assert(flipAssign != null && clampAssign != null, "both claw functions should have got a button");
        ValidationUtil.Assert(flipAssign.mode == ControllerMapSettings.ModeToggle &&
        ...
        ValidationUtil.Assert(flipAssign.button != clampAssign.button,
            "flip and clamp must land on DIFFERENT buttons or one is unreachable");
        ValidationUtil.Assert(FindAssignment(map, followerId) == null,
        ...
        ValidationUtil.Assert(f.jawB.GetComponent<ArticulationBody>() == null,
            "dropping a clamp half must remove its joint, or it stays a live orphan that still moves");
        ValidationUtil.Assert(f.jawB.GetComponent<JointCoupler>() == null,
            "dropping a clamp half must remove its coupler");
        ValidationUtil.Assert(f.registry.Find(clampId) != null,
        ...
        ValidationUtil.Assert(rig != null, "the build should leave a ClawRig record so the claw can be re-edited");
        ClawSetup.Strip(f.registry, rig, useUndo: false);

        ValidationUtil.Assert(f.registry.Find(flipId) == null && f.registry.Find(clampId) == null,
            "Strip must remove both claw mechanisms from the registry");
        ValidationUtil.Assert(f.flip.GetComponent<ArticulationBody>() == null &&
               f.jawA.GetComponent<ArticulationBody>() == null,
            "Strip must return the claw's links to plain welded geometry");
        ValidationUtil.Assert(f.registry.GetComponentInChildren<ClawGrab>(true) == null, "Strip must remove the grab");
        ValidationUtil.Assert(f.registry.GetComponentInChildren<ClawRig>(true) == null, "Strip must remove its own record");
        foreach (Collider c in f.clampRod.GetComponentsInChildren<Collider>(true))
            ValidationUtil.Assert(c.enabled, "Strip must give a cosmetic cylinder its colliders back");
        ButtonMap after = ControllerMapSettings.Load(TestRobotId);
        ValidationUtil.Assert(FindAssignment(after, flipId) == null && FindAssignment(after, clampId) == null,
    ...
    private static string TrimMovesTheRestPose()
    {
        ...
        ValidationUtil.Assert(driver != null && follower != null && coupler != null && piston != null,
        ...
        ValidationUtil.Near(driver.xDrive.lowerLimit, Trim, 0.5f,
            $"a {Trim} trim should carry the jaws' resting pose {-Trim} degrees past the CAD pose");
        ValidationUtil.Near(driver.xDrive.upperLimit, CloseAngle + Trim, 0.5f,
            "trimming the rest pose must move the whole travel, not lengthen it");
        ValidationUtil.Near(driver.xDrive.upperLimit - driver.xDrive.lowerLimit, CloseAngle, 0.5f,
            "the jaw should still swing exactly its close angle after a trim");
        ValidationUtil.Near(piston.retractedTarget, Trim, 0.5f,
        ...
        ValidationUtil.Near(coupler.offsetDeg, 0f, 0.01f,
        ...
        ValidationUtil.Near(restDriver, Trim, 2f,
            $"the driven jaw should rest at the trimmed pose ({Trim}) but sat at {restDriver:F1}");
        ValidationUtil.Near(restFollower, -Trim, 2f,
        ...
        ValidationUtil.Near(firedDriver, CloseAngle + Trim, 2f,
    ...
    private static string FlipTurnsInPlace()
    {
        ...
        Transform pivot = MechanismBuildUtil.FindChild(f.registry.transform, "ClawFlipPivot");
        ValidationUtil.Assert(pivot != null, "the build should have created a flip pivot marker");
        ...
        ValidationUtil.Assert(offset < span * 0.05f,
        ...
        ValidationUtil.Assert(plateOnly > span * 0.05f,
        ...
        ValidationUtil.Assert(holdOffset < jawsOnly.size.magnitude * 0.25f,
    ...
    private static string CarriedPiecesLandRight()
    {
        ...
        ClawSetup.Build(f.Options(), useUndo: false);
        ClawGrab grab = f.registry.GetComponentInChildren<ClawGrab>(true);
        ValidationUtil.Assert(grab != null && grab.holdPoint != null, "the build should have wired a grab and hold point");
        ValidationUtil.Assert(grab.autoUpright,
        ...
        GameObject pin = ValidationUtil.MakeBox(null, "PinPiece", new Vector3(40f, 5f, 0f), Vector3.one);
        ...
        ValidationUtil.Assert(comOffset > 1f,
        ...
        ValidationUtil.Assert(offRobotUp < 1f,
        ...
        ValidationUtil.Assert(miss < 0.01f,
                 ...
                 })
        {
            ...
            ValidationUtil.Near(swung, expectedSwing, 1f,
        ...
        ValidationUtil.Assert(rides < 1f,
        ...
        ValidationUtil.Near(worst, StackHeight * 0.5f, 0.01f,
        ...
        ValidationUtil.Near(heavy.y, StackHeight * 0.25f, 0.01f,
        ...
        ValidationUtil.Assert(alone.magnitude < 0.001f, "one piece on its own belongs on the hold point, unshifted");
    ...
    private static string ExplicitAxesMeanWhatTheySay()
    {
        ...
        ValidationUtil.Near(Mathf.Abs(Vector3.Dot(sceneX, Vector3.right)), 1f, 0.01f,
            "'Scene X (red)' must hinge about the scene's red arrow, whatever frame the part is in");
        ValidationUtil.Near(Mathf.Abs(Vector3.Dot(sceneY, Vector3.up)), 1f, 0.01f,
            "'Scene Y (green)' must hinge about the scene's green arrow");
        ValidationUtil.Near(Mathf.Abs(Vector3.Dot(sceneZ, Vector3.forward)), 1f, 0.01f,
        ...
        ValidationUtil.Assert(apart > 30f,
        ...
        ValidationUtil.Near(Mathf.Abs(Vector3.Dot(flipAuto, robotForward)), 1f, 0.01f,
        ...
        ValidationUtil.Near(Mathf.Abs(Vector3.Dot(clampAuto, f.registry.transform.up)), 1f, 0.01f,
    ...
    private static string LevelKeeperStaysLevel()
    {
        ...
        ValidationUtil.Assert(mountBody != null && mountBody.jointType == ArticulationJointType.RevoluteJoint,
            "the mount should have become a revolute joint");
        ValidationUtil.Assert(mount.transform.parent == arm.transform,
        ...
        ValidationUtil.Assert(flip.transform.IsChildOf(mount.transform),
            "the claw (flip link) must ride the mount, or it wouldn't be kept level");
        ValidationUtil.Assert(f.jawA.transform.IsChildOf(mount.transform) && f.jawB.transform.IsChildOf(mount.transform),
            "the JAWS must end up under the mount too, or they stay rigid to the arm (the reported bug)");
        ValidationUtil.Assert(mount.GetComponent<PneumaticActuator>() == null && mount.GetComponent<MotorActuator>() == null,
            "the mount is a passive linkage — an actuator would let ButtonRouter fight the coupler");
        ValidationUtil.Assert(registry.Find(UrdfPostProcessor.Slugify(mount.name)) == null,
            "the mount must not be registered as a mechanism (it rides no button)");
        ValidationUtil.Assert(coupler != null && coupler.mode == JointCoupler.CoupleMode.Position,
            "the mount should track the arm's ANGLE through a Position coupler");
        ValidationUtil.Assert(coupler.driver == armBody && coupler.follower == mountBody,
        ...
        ValidationUtil.Assert(!coupler.flipPastMidpoint, "the plain level-keeper must not flip unless it's asked to");
        ...
        ValidationUtil.Assert(pivToClaw < pivToMountOrigin,
        ...
        foreach (float checkpoint in new[] { 45f, 90f, 135f, 180f })
        {
            ...
            ValidationUtil.Assert(Mathf.Abs(armReached - checkpoint) < 8f,
        ...
        ValidationUtil.Assert(farthest > 1f,
        ...
        ValidationUtil.Assert(worstTilt < 5f,
        ...
        ValidationUtil.Assert(mutantTilt > 30f,
    ...
    private static string LevelKeeperFlipsPastMidpoint()
    {
        ...
        JointCoupler coupler = f.coupler;
        ArticulationBody armBody = f.armBody, mountBody = f.mountBody;
        ValidationUtil.Assert(coupler != null && coupler.flipPastMidpoint,
            "the flip option should have reached the built coupler");
        ValidationUtil.Near(coupler.flipDegrees, 180f, 1e-3f, "the flip should be the 180 that was asked for");
        ...
        ValidationUtil.Assert(flipped > 150f,
        ...
        ValidationUtil.Assert(nfMoved < 15f,
    ...
    private static string LevelKeeperYawsWithWrist()
    {
        ...
        ValidationUtil.Assert(f.wristBody != null && f.wristCoupler != null,
            "the wrist part should have become a joint with a coupler");
        ValidationUtil.Assert(f.flip.transform.IsChildOf(f.wrist.transform),
            "the claw must ride the WRIST, so the yaw carries it");
        ValidationUtil.Assert(f.wrist.transform.IsChildOf(f.mount.transform),
            "the wrist must ride the MOUNT, so the leveling carries both");
        ValidationUtil.Assert(!f.coupler.flipPastMidpoint,
            "with a wrist present the MOUNT must not flip — it only levels; the wrist yaws");
        ValidationUtil.Assert(f.wristCoupler.flipPastMidpoint && Mathf.Abs(f.wristCoupler.ratio) < 1e-3f,
        ...
        ValidationUtil.Assert(unyawed < 12f,
        ...
        ValidationUtil.Assert(fromStart > 150f, $"the claw only turned {fromStart:F0} degrees — it didn't face the other way");
        ValidationUtil.Assert(upTilt < 15f,
        ...
        ValidationUtil.Assert(pitchUpTilt > 90f,
    ...
    private static string FlipOffReleasesTheWrist()
    {
        ...
        Transform wristMarker = MechanismBuildUtil.FindChild(f.wrist.transform, "ClawYawWristPivot");
        ValidationUtil.Assert(wristMarker != null, "the first build should have dropped a wrist pivot marker");
        ...
        ValidationUtil.Assert(f.wrist.GetComponent<ArticulationBody>() == null,
        ...
        ValidationUtil.Assert(f.wrist.GetComponent<JointCoupler>() == null,
            "the wrist kept its coupler — a leftover coupler would still turn it at the midpoint");
        ValidationUtil.Assert(wristMarker == null, "the released wrist kept its pivot marker");
        ...
        ValidationUtil.Assert(mountBody != null && coupler != null, "the mount must still be the level-keeper joint");
        ValidationUtil.Assert(!coupler.flipPastMidpoint,
            "the mount picked the flip up instead — turning the flip off must leave NOTHING flipping");
        ValidationUtil.Assert(f.flip.transform.IsChildOf(f.mount.transform),
        ...
        ValidationUtil.Assert(turned < 15f,
        ...
        ValidationUtil.Assert(mutantTurned > 150f,
    ...
    private static string LevelKeeperRidesTheAxle()
    {
        ...
        ArticulationBody armBody = f.armBody, mountBody = f.mountBody;
        JointCoupler coupler = f.coupler;
        ValidationUtil.Assert(f.axle != null, "the axle fixture should have created an axle part");
        ...
        ValidationUtil.Assert(pivToAxle < 0.2f,
        ...
        ValidationUtil.Assert(axleToMountOrigin > 0.5f,
        ...
        foreach (float checkpoint in new[] { 45f, 90f, 135f, 180f })
        {
            ...
            ValidationUtil.Assert(Mathf.Abs(JointAngleDeg(armBody) - checkpoint) < 8f,
        ...
        ValidationUtil.Assert(wobble < 1f,
        ...
        ValidationUtil.Assert(mutantWobble > 2f,
    ...
    private static string FlipRollsAboutTheAxle()
    {
        ArmFixture f = MakeSwingArmClaw("RollBot", flipPastMidpoint: true, withWrist: true, withAxle: true,
            axleUnderMount: true);
        ValidationUtil.Assert(f.axle != null && f.wristBody != null, "the fixture should have an axle and a wrist joint");

        // (a) Welded to the ARM — a DIRECT child of the arm, pulled out of the mount it was nested on.
        ValidationUtil.Assert(f.axle.transform.parent == f.arm.transform,
        ...
        ValidationUtil.Assert(maxD - minD < 0.5f,
    ...
    private static string RebuildFollowsMovedAxle()
    {
        ...
        ValidationUtil.Assert(Vector3.Distance(anchor0, axle0) < 0.2f,
        ...
        ValidationUtil.Assert(Vector3.Distance(axle0, axle1) > 1f,
        ...
        ValidationUtil.Assert(Vector3.Distance(anchor1, axle1) < 0.2f,
        ...
        ValidationUtil.Assert(Vector3.Distance(anchor1, axle0) > 1f,
        ...
        bool withAxle = false, bool forceLevelPivotAtRootCentre = false, bool axleUnderMount = false)
    {
        ...
        GameObject arm = ValidationUtil.MakeBox(root.transform, "SwingArm", new Vector3(6f, 3f, 0f), new Vector3(1f, 1f, 5f));
        GameObject mount = ValidationUtil.MakeBox(root.transform, "ClawMount", new Vector3(8f, 3f, 0f), new Vector3(1.5f, 1.5f, 1.5f));
        GameObject flip = ValidationUtil.MakeBox(root.transform, "ClawFlipAssembly", new Vector3(9f, 3f, 0f), new Vector3(2f, 1f, 3f));
        GameObject jawA = ValidationUtil.MakeBox(root.transform, "ClawJawLeft", new Vector3(10f, 3f, -1.2f), new Vector3(2f, 0.6f, 0.4f));
        GameObject jawB = ValidationUtil.MakeBox(root.transform, "ClawJawRight", new Vector3(10f, 3f, 1.2f), new Vector3(2f, 0.6f, 0.4f));
        ...
        GameObject wrist = withWrist
            ? ValidationUtil.MakeBox(root.transform, "ClawWrist", new Vector3(8.6f, 3f, 0f), new Vector3(1f, 1f, 1f)) : null;
        ...
        GameObject axle = withAxle
            ? ValidationUtil.MakeBox(root.transform, "ClawAxle", new Vector3(8.5f, 3f, 0f), new Vector3(0.3f, 0.3f, 2.2f))
            : null;
        ...
        ValidationUtil.Assert(armBody != null && armBody.jointType == ArticulationJointType.RevoluteJoint,
    ...
    private static string JawsCloseAgainstEachOther()
    {
        ...
        ValidationUtil.Near(Mathf.Abs(followerDeg), Mathf.Abs(driverDeg), 2f,
        ...
        ValidationUtil.Near(Mathf.Abs(driverDeg), CloseAngle, 2f,
        ...
        ValidationUtil.Assert(jawTravel.magnitude > 1e-3f,
            "the jaw's geometry never moved, so which way it hinges can't be read");
        ValidationUtil.Assert(sideways > upward * 2f,
        ...
        // jaws move.
        PneumaticSlideFollower rod = f.clampRod.GetComponent<PneumaticSlideFollower>();
        ValidationUtil.Assert(rod.Progress01() > 0.5f,
    ...
    private static string RejectionsHold()
    {
        ...
        ValidationUtil.Assert(f.registry.mechanisms.Count == 0,
            "a rejected build must leave the robot untouched — no half-registered mechanisms");
        ValidationUtil.Assert(f.jawA.GetComponent<ArticulationBody>() == null,
    ...
    private static Fixture MakeFixture(string name)
    {
        ...
        f.flip = ValidationUtil.MakeBox(root.transform, "ClawFlipAssembly", new Vector3(5f, 2f, 0f), new Vector3(2f, 1f, 3f));
        f.jawA = ValidationUtil.MakeBox(root.transform, "ClawJawLeft", new Vector3(7f, 2f, -1.2f), new Vector3(2f, 0.6f, 0.4f));
        f.jawB = ValidationUtil.MakeBox(root.transform, "ClawJawRight", new Vector3(7f, 2f, 1.2f), new Vector3(2f, 0.6f, 0.4f));

        f.flipBody = ValidationUtil.MakeBox(root.transform, "FlipCylinderBody", new Vector3(2f, 3f, 0f), new Vector3(1.5f, 0.4f, 0.4f));
        f.flipRod = ValidationUtil.MakeBox(root.transform, "FlipCylinderRod", new Vector3(3.5f, 3f, 0f), new Vector3(1f, 0.2f, 0.2f));
        f.clampBody = ValidationUtil.MakeBox(root.transform, "ClampCylinderBody", new Vector3(5f, 3f, 0f), new Vector3(1.2f, 0.3f, 0.3f));
        f.clampRod = ValidationUtil.MakeBox(root.transform, "ClampCylinderRod", new Vector3(6.2f, 3f, 0f), new Vector3(0.8f, 0.15f, 0.15f));
        return f;
    }
