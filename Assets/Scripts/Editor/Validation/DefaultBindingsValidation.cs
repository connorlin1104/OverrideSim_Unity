...
public static class DefaultBindingsValidation
{
    ...
    {
        ValidationUtil.RunBatch("Validate Default Bindings", Run);
    }
    ...
    private static int CloneIsDeep()
    {
        ...
        ValidationUtil.Assert(!ReferenceEquals(copy, source), "Clone must return a new map");
        ValidationUtil.Assert(!ReferenceEquals(copy.assignments, source.assignments), "Clone must copy the assignment list");
        ValidationUtil.Assert(!ReferenceEquals(copy.styles, source.styles), "Clone must copy the style list");
        ValidationUtil.Assert(copy.assignments.Count == source.assignments.Count, "Clone must keep every assignment");
        ValidationUtil.Assert(copy.styles.Count == source.styles.Count, "Clone must keep every style");
        ...
        ValidationUtil.Assert(source.assignments[0].mechanismId == "intake",
            "mutating a clone's assignment must not reach the original");
        ValidationUtil.Assert(source.assignments.Count == 1, "adding to a clone must not reach the original");
        ValidationUtil.Assert(source.styles.Count == 1, "removing a clone's style must not reach the original");

        ValidationUtil.Assert(ControllerMapSettings.Clone(null) != null, "Clone(null) must return an empty map, not null");
        return 9;
    }
    ...
    private static int FreshDeviceGetsTheDefault()
    {
        ...
        ValidationUtil.Assert(ControllerMapSettings.SeedDefault(entry), "a device with no layout must be seeded");
        ButtonMap loaded = ControllerMapSettings.Load(TestRobotId);
        ValidationUtil.Assert(loaded.assignments.Count == 2, "the seeded layout should have both buttons");
        ...
        ValidationUtil.Assert(!ControllerMapSettings.SeedDefault(entry),
    ...
    private static int CustomisedLayoutSurvives()
    {
        ...
        ValidationUtil.Assert(!ControllerMapSettings.SeedDefault(entry), "a customised layout must not be re-seeded");
        ...
        ValidationUtil.Assert(ControllerMapSettings.Find(after, ControllerButton.R1) == null,
    ...
    private static int ClearedLayoutStaysCleared()
    {
        ...
        ValidationUtil.Assert(ControllerMapSettings.Load(TestRobotId).assignments.Count == 0,
    ...
    private static int UntouchedLayoutTakesAnUpdate()
    {
        ...
        ValidationUtil.Assert(!ControllerMapSettings.SeedDefault(entry), "the update must itself only land once");
        return 4;
    }
    ...
    private static int NoDefaultIsANoOp()
    {
        ...
        ValidationUtil.Assert(!entry.HasDefaultButtonMap, "a new entry must not claim to have a default");
        ValidationUtil.Assert(!ControllerMapSettings.SeedDefault(entry), "an entry with no default must not seed");
        ValidationUtil.Assert(!PlayerPrefs.HasKey(ControllerMapSettings.PrefKey(TestRobotId)),
            "seeding nothing must not create a map pref");
        ValidationUtil.Assert(!PlayerPrefs.HasKey(ControllerMapSettings.SeedPrefKey(TestRobotId)),
        ...
        ValidationUtil.Assert(!entry.HasDefaultButtonMap, "an empty default must not count as a default");
        ValidationUtil.Assert(!ControllerMapSettings.ResetToDefault(entry), "there is nothing to reset to");
        return 6;
    }
    ...
    private static int ResetAlwaysOverwrites()
    {
        ...
        ValidationUtil.Assert(ControllerMapSettings.ResetToDefault(entry), "reset must report that it wrote");
        ButtonMap after = ControllerMapSettings.Load(TestRobotId);
        AssertHas(after, ControllerButton.R1, "intake", ControllerMapSettings.ModeForward);
        ValidationUtil.Assert(ControllerMapSettings.Find(after, ControllerButton.Y) == null,
    ...
    {
        ValidationUtil.Assert(!ControllerMapSettings.SeedDefault(null), "SeedDefault(null) must be a no-op");
        ValidationUtil.Assert(!ControllerMapSettings.ResetToDefault(null), "ResetToDefault(null) must be a no-op");
        ValidationUtil.Assert(ControllerMapSettings.SeedDefaults(null) == 0, "SeedDefaults(null) must be a no-op");
        ...
        try
        {
            ...
            ValidationUtil.Assert(ControllerMapSettings.SeedDefaults(catalog) == 0,
    ...
    private static void AssertHas(ButtonMap map, ControllerButton button, string id, string mode)
        => ValidationUtil.Assert(ControllerMapSettings.HasAssignment(map, button, id, mode),
